...
function Frame({ toggleStates, onToggle, labels, gap = 80, onPreTreatmentToggle }: { 
    ...
    <div className="content-stretch flex items-center justify-start relative shrink-0 w-full" style={{ gap: `${gap}px` }}>
      {labels.map((label, index) => (
        <Toggle 
          ...
          onToggle={() => {
            onToggle(index);
            if (label === "Pre-treatment" && onPreTreatmentToggle) {
              onPreTreatmentToggle(!toggleStates[index]);
            }
          }}
        />
      ))}
...
export default function Rx({ variant = 'default', labels, initialStates, onPreTreatmentToggle }: RxProps) {
  // Define variants
  const variantConfig = {
    default: {
      labels: [
    ...
    <div className="bg-white relative rounded-[8px] size-full" data-name="Rx">
      ...
      <div className="flex flex-col justify-center size-full">
        <div className="content-stretch flex flex-col items-start justify-center p-[24px] relative shrink-0">
          <Frame toggleStates={toggleStates} onToggle={handleToggle} labels={toggleLabels} gap={config.gap} onPreTreatmentToggle={onPreTreatmentToggle} />
        </div>
      </div>
    </div>
  );
